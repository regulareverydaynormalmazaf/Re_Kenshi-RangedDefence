// ============================================================================
//  RangedDefence v10 GOG  —  KenshiLib plugin (Kenshi 1.0.65 Steam).
//
...
struct State {
    ...
    int32_t selection_mode;      // SelMode
    int32_t allow_block_fallback;// dodge-primary: try block after failed dodge
    float   chance_multiplier;   // scale evasion chance (0..3)
    int32_t xp_enabled;          // grant dodge/block XP on attempt
    int32_t heavy_category;      // WeaponCategory int treated as "heavy" (SKILL_HEAVY = 3)
    int32_t unarmed_can_block;   // no-melee/ranged-only: allow blocking arrows too (else dodge-only)
    ...
    int32_t turret_mode;         // what a turret operator does: 0 = no defence (default), 1 = dodge, 2 = block
    int32_t use_weapon_weight;   // «По оружию»: 0 = heavy by CATEGORY (heavy_category); 1 = heavy by combat WEIGHT
    int32_t heavy_weight;        // weight threshold: getCombatWeight() >= this -> heavy (only when use_weapon_weight = 1)
...
// (cooldown_enabled appended at the END so existing positional configs keep their meaning)
static State g_state = { 1, SEL_HYBRID, 1, 1.0f, 1, 3, 0, 500, 900, 1, 1, 0, 0, 0, 0, 0, 10, 1 };
enum TurretMode { TURRET_NONE = 0, TURRET_DODGE = 1, TURRET_BLOCK = 2 };
...
static void LoadConfig() {
    ...
    if (findNumber(s, "selection_mode", v))        g_state.selection_mode = (int)v;
    if (findNumber(s, "allow_block_fallback", v))  g_state.allow_block_fallback = (v != 0);
    if (findNumber(s, "chance_multiplier", v))     g_state.chance_multiplier = (float)v;
    if (findNumber(s, "xp_enabled", v))            g_state.xp_enabled = (v != 0);
    if (findNumber(s, "heavy_category", v))        g_state.heavy_category = (int)v;
    if (findNumber(s, "unarmed_can_block", v))     g_state.unarmed_can_block = (v != 0);
    ...
    if (findNumber(s, "turret_mode", v))           g_state.turret_mode = (int)v;
    if (findNumber(s, "use_weapon_weight", v))     g_state.use_weapon_weight = (v != 0);
    if (findNumber(s, "heavy_weight", v))          g_state.heavy_weight = (int)v;
    ...
    if (g_state.selection_mode < 0 || g_state.selection_mode > 5) g_state.selection_mode = SEL_HYBRID;
    if (g_state.turret_mode < 0 || g_state.turret_mode > 2) g_state.turret_mode = TURRET_NONE;
    if (g_state.heavy_weight < 0)  g_state.heavy_weight = 0;
    if (g_state.heavy_weight > 50) g_state.heavy_weight = 50;
...
static void writeConfigTo(std::ostream& f) {
    f << "{\n"
      ...
      << "  \"selection_mode\": "        << g_state.selection_mode << ",\n"
      << "  \"allow_block_fallback\": "  << (g_state.allow_block_fallback ? "true" : "false") << ",\n"
      << "  \"chance_multiplier\": "     << g_state.chance_multiplier << ",\n"
      << "  \"xp_enabled\": "            << (g_state.xp_enabled ? "true" : "false") << ",\n"
      << "  \"heavy_category\": "        << g_state.heavy_category << ",\n"
      << "  \"unarmed_can_block\": "     << (g_state.unarmed_can_block ? "true" : "false") << ",\n"
      << "  \"dodge_lock_ms\": "         << g_state.dodge_lock_ms << ",\n"
      ...
      << "  \"turret_mode\": "           << g_state.turret_mode << ",\n"
      << "  \"use_weapon_weight\": "     << (g_state.use_weapon_weight ? "true" : "false") << ",\n"
      << "  \"heavy_weight\": "          << g_state.heavy_weight << ",\n"
...
bool iShotYou_hook(Character* thisptr, Character* attacker, Harpoon* poon, bool onPurpose) {
    ...
    if (g_state.enabled && thisptr && attacker && thisptr != attacker
        ...
        if (onTurret && g_state.turret_mode == TURRET_NONE) {
            if (rdlogActive()) rdlog("iShotYou -> target is using a turret; turret_mode=none -> no defence.");
        ...
        if (ps != PS_KO && ps != PS_PLAYING_DEAD) {
            ...
            if (def && atk) {
                ...
                // «тяжёлое» для режима «По оружию»: по ВЕСУ (getCombatWeight >= порог) если включён
                // use_weapon_weight, иначе по КАТЕГОРИИ (cat == heavy_category). Оба источника — по RVA.
                bool heavyWeapon = hasWeapon && (g_state.use_weapon_weight
                                                 ? (wt >= (float)g_state.heavy_weight)
                                                 : (cat == g_state.heavy_category));
                // «без ближнего оружия»: пусто, безоружный, лук или турель (дальнобойное в руках)
                bool noMelee = (!hasWeapon) || cat == WC_UNARMED || cat == WC_BOW || cat == WC_TURRET;
                ...
                if (onTurret) {
                    // turret operator (turret_mode != none): forced defence type, no fallback
                    primaryBlock = (g_state.turret_mode == TURRET_BLOCK);
                } else if (noMelee) {
                    // без ближнего оружия / с дальнобойным -> по умолчанию УКЛОН.
                    // блокировать стрелы можно лишь если включён unarmed_can_block.
                    if (g_state.selection_mode == SEL_BLOCK_ONLY && g_state.unarmed_can_block) {
                        primaryBlock = true;
                    } else {
                        primaryBlock = false;
                        fallbackBlock = (g_state.unarmed_can_block
                                         && g_state.selection_mode != SEL_DODGE_ONLY
                                         && g_state.allow_block_fallback);
                    }
                } else {
                    switch (g_state.selection_mode) {
                        case SEL_BY_STAT:    primaryBlock = (str > dex);  break;
                        case SEL_DODGE_ONLY: primaryBlock = false; break;
                        case SEL_BLOCK_ONLY: primaryBlock = true;  break;
                        case SEL_HYBRID:
                        default:             primaryBlock = heavyWeapon; break;  // «По оружию»: метка решает
                    }
                    // «Блок после уклона» — единственный источник фолбэка; работает только для уклон-основы.
                    if (!primaryBlock && g_state.selection_mode != SEL_DODGE_ONLY && g_state.allow_block_fallback)
                        fallbackBlock = true;
                }
                ...
                if (rdlogActive()) {
                    ...
                    o << "iShotYou this=" << (void*)thisptr << " shooter=" << shooter
                      << " cat=" << cat << (heavyWeapon ? "(HEAVY)" : "") << " wt=" << wt
                      << " useWt=" << g_state.use_weapon_weight << "/" << g_state.heavy_weight
                      << " str=" << str << " dex=" << dex << " opp=" << opp << " atk=" << atkf
                      << " turret=" << (onTurret ? g_state.turret_mode : -1) << " noMelee=" << (int)noMelee
                      << " mode=" << g_state.selection_mode << " primary=" << (primaryBlock ? "block" : "dodge")
...
// Two static option tables (values identical; labels localized). Static storage -> safe for the hub.
static const EMC_SelectOptionV1 kModeOptions_en[] = {
    { SEL_HYBRID,     "By weapon" },
    { SEL_BY_STAT,    "STR / DEX" },
    { SEL_DODGE_ONLY, "Dodge only" },
    { SEL_BLOCK_ONLY, "Block only" },
};
static const EMC_SelectOptionV1 kModeOptions_ru[] = {
    { SEL_HYBRID,     "\xd0\x9f\xd0\xbe \xd0\xbe\xd1\x80\xd1\x83\xd0\xb6\xd0\xb8\xd1\x8e" },
    { SEL_BY_STAT,    "\xd0\xa1\xd0\xb8\xd0\xbb\xd0\xb0/\xd0\xbb\xd0\xbe\xd0\xb2\xd0\xba\xd0\xbe\xd1\x81\xd1\x82\xd1\x8c" },
    { SEL_DODGE_ONLY, "\xd0\xa2\xd0\xbe\xd0\xbb\xd1\x8c\xd0\xba\xd0\xbe \xd1\x83\xd0\xba\xd0\xbb\xd0\xbe\xd0\xbd" },
    { SEL_BLOCK_ONLY, "\xd0\xa2\xd0\xbe\xd0\xbb\xd1\x8c\xd0\xba\xd0\xbe \xd0\xb1\xd0\xbb\xd0\xbe\xd0\xba" },
};
// Turret-operator behaviour options (values = TurretMode).
static const EMC_SelectOptionV1 kTurretOptions_en[] = {
    { TURRET_NONE,  "No defence" },
    { TURRET_DODGE, "Dodge" },
    { TURRET_BLOCK, "Block" },
};
static const EMC_SelectOptionV1 kTurretOptions_ru[] = {
    { TURRET_NONE,  "\xd0\x9d\xd0\xb5 \xd0\xb7\xd0\xb0\xd1\x89\xd0\xb8\xd1\x89\xd0\xb0\xd0\xb5\xd1\x82\xd1\x81\xd1\x8f" },
    { TURRET_DODGE, "\xd0\xa3\xd0\xba\xd0\xbb\xd0\xbe\xd0\xbd" },
    { TURRET_BLOCK, "\xd0\x91\xd0\xbb\xd0\xbe\xd0\xba" },
};
...
// ---- V2-aware registration helpers ---------------------------------------------------------
// Attach a hover tooltip (hover_hint) when the hub supports the V2 row (bool/select only); else
// fall back to V1 without a tooltip. The tooltip is a floating box shown over the control on hover.
// int/float rows have no hover variant in the hub API, so they get a short inline description only.
static void regBool(const EMC_HubApiV1* api, uint32_t apiSize, EMC_ModHandle mod,
                    const char* id, const char* label, const char* desc, const char* hover, int32_t* field) {
    if (hover && hover[0] && apiSize >= EMC_HUB_API_V1_BOOL_SETTING_V2_MIN_SIZE && api->register_bool_setting_v2) {
        EMC_BoolSettingDefV2 d = { id, label, desc, field, &cbGetBool, &cbSetBool, hover };
        api->register_bool_setting_v2(mod, &d);
    } else {
...
static void regSelect(const EMC_HubApiV1* api, uint32_t apiSize, EMC_ModHandle mod,
                      const char* id, const char* label, const char* desc, const char* hover,
                      int32_t* field, const EMC_SelectOptionV1* opts, uint32_t count) {
    if (hover && hover[0] && apiSize >= EMC_HUB_API_V1_SELECT_SETTING_V2_MIN_SIZE && api->register_select_setting_v2) {
        EMC_SelectSettingDefV2 d = { id, label, desc, field, opts, count, &cbGetSel, &cbSetSel, hover };
        api->register_select_setting_v2(mod, &d);
    } else if (apiSize >= EMC_HUB_API_V1_SELECT_SETTING_MIN_SIZE && api->register_select_setting) {
...
static bool DoRegisterModHub() {
    ...
    g_modDesc.mod_display_name       = tr("Ranged Defence (v10 GOG)", "\xd0\x94\xd0\xb0\xd0\xbb\xd1\x8c\xd0\xbd\xd1\x8f\xd1\x8f \xd0\xb7\xd0\xb0\xd1\x89\xd0\xb8\xd1\x82\xd0\xb0 (v10 GOG)");
    ...
    regBool(api, apiSize, mod, "enabled",
        ...
        tr("Master switch for ranged dodge/block for ALL characters (player and NPC).\nOff = the mod does nothing; arrows behave as in vanilla.",
           "\xd0\x93\xd0\xbb\xd0\xb0\xd0\xb2\xd0\xbd\xd1\x8b\xd0\xb9 \xd0\xb2\xd1\x8b\xd0\xba\xd0\xbb\xd1\x8e\xd1\x87\xd0\xb0\xd1\x82\xd0\xb5\xd0\xbb\xd1\x8c \xd1\x83\xd0\xba\xd0\xbb\xd0\xbe\xd0\xbd\xd0\xb5\xd0\xbd\xd0\xb8\xd1\x8f/\xd0\xb1\xd0\xbb\xd0\xbe\xd0\xba\xd0\xb0 \xd0\xbe\xd1\x82 \xd0\xb4\xd0\xb0\xd0\xbb\xd1\x8c\xd0\xbd\xd0\xb8\xd1\x85 \xd0\xb0\xd1\x82\xd0\xb0\xd0\xba \xd0\xb4\xd0\xbb\xd1\x8f \xd0\x92\xd0\xa1\xd0\x95\xd0\xa5 \xd0\xbf\xd0\xb5\xd1\x80\xd1\x81\xd0\xbe\xd0\xbd\xd0\xb0\xd0\xb6\xd0\xb5\xd0\xb9 (\xd0\xb8\xd0\xb3\xd1\x80\xd0\xbe\xd0\xba \xd0\xb8 \xd0\x9d\xd0\x9f\xd0\xa1).\n\xd0\x92\xd1\x8b\xd0\xba\xd0\xbb = \xd0\xbc\xd0\xbe\xd0\xb4 \xd0\xbd\xd0\xb8\xd1\x87\xd0\xb5\xd0\xb3\xd0\xbe \xd0\xbd\xd0\xb5 \xd0\xb4\xd0\xb5\xd0\xbb\xd0\xb0\xd0\xb5\xd1\x82, \xd1\x81\xd1\x82\xd1\x80\xd0\xb5\xd0\xbb\xd1\x8b \xd0\xbb\xd0\xb5\xd1\x82\xd1\x8f\xd1\x82 \xd0\xba\xd0\xb0\xd0\xba \xd0\xb2 \xd0\xb2\xd0\xb0\xd0\xbd\xd0\xb8\xd0\xbb\xd0\xb8."),
        &g_state.enabled);

    {
        const EMC_SelectOptionV1* opts = (g_lang == LANG_RU) ? kModeOptions_ru : kModeOptions_en;
        regSelect(api, apiSize, mod, "selection_mode",
            tr("Selection mode", "\xd0\xa0\xd0\xb5\xd0\xb6\xd0\xb8\xd0\xbc \xd0\xb2\xd1\x8b\xd0\xb1\xd0\xbe\xd1\x80\xd0\xb0"),
            tr("How the primary defence is chosen.", "\xd0\x9a\xd0\xb0\xd0\xba \xd0\xb2\xd1\x8b\xd0\xb1\xd0\xb8\xd1\x80\xd0\xb0\xd0\xb5\xd1\x82\xd1\x81\xd1\x8f \xd0\xbe\xd1\x81\xd0\xbd\xd0\xbe\xd0\xb2\xd0\xbd\xd0\xb0\xd1\x8f \xd0\xb7\xd0\xb0\xd1\x89\xd0\xb8\xd1\x82\xd0\xb0."),
x8c \xc2\xab\xd0\x91\xd0\xbb\xd0\xbe\xd0\xba \xd0\xbf\xd0\xbe\xd1\x81\xd0\xbb\xd0\xb5 \xd1\x83\xd0\xba\xd0\xbb\xd0\xbe\xd0\xbd\xd0\xb0\xc2\xbb \xd0\xb4\xd0\xbe\xd0\xb1\xd0\xb0\xd0\xb2\xd0\xbb\xd1\x8f\xd0\xb5\xd1\x82 \xd0\xb1\xd0\xbb\xd0\xbe\xd0\xba, \xd0\xb5\xd1\x81\xd0\xbb\xd0\xb8 \xd0\xbe\xd1\x81\xd0\xbd\xd0\xbe\xd0\xb2\xd0\xbd\xd0\xbe\xd0\xb9 \xd0\xb1\xd1\x8b\xd0\xbb \xd1\x83\xd0\xba\xd0\xbb\xd0\xbe\xd0\xbd."),
            &g_state.selection_mode, opts, (uint32_t)(sizeof(kModeOptions_en)/sizeof(kModeOptions_en[0])));
    }

    regBool(api, apiSize, mod, "use_weapon_weight",
        tr("Use weapon weight", "\xd0\x98\xd1\x81\xd0\xbf\xd0\xbe\xd0\xbb\xd1\x8c\xd0\xb7\xd0\xbe\xd0\xb2\xd0\xb0\xd1\x82\xd1\x8c \xd0\xb2\xd0\xb5\xd1\x81 \xd0\xbe\xd1\x80\xd1\x83\xd0\xb6\xd0\xb8\xd1\x8f"),
        tr("\xc2\xab" "By weapon\xc2\xbb mode: heavy = by combat weight, not by category.",
           "\xd0\xa0\xd0\xb5\xd0\xb6\xd0\xb8\xd0\xbc \xc2\xab\xd0\x9f\xd0\xbe \xd0\xbe\xd1\x80\xd1\x83\xd0\xb6\xd0\xb8\xd1\x8e\xc2\xbb: \xd1\x82\xd1\x8f\xd0\xb6\xd1\x91\xd0\xbb\xd0\xbe\xd0\xb5 \xe2\x80\x94 \xd0\xbf\xd0\xbe \xd0\xb2\xd0\xb5\xd1\x81\xd1\x83, \xd0\xb0 \xd0\xbd\xd0\xb5 \xd0\xbf\xd0\xbe \xd0\xba\xd0\xb0\xd1\x82\xd0\xb5\xd0\xb3\xd0\xbe\xd1\x80\xd0\xb8\xd0\xb8."),
        tr("Only affects the 'By weapon' mode. Off = heavy is decided by weapon CATEGORY (id below).\nOn = heavy is decided by combat WEIGHT (>= the threshold below) instead of category. It replaces category, not combined with it.",
           "\xd0\x92\xd0\xbb\xd0\xb8\xd1\x8f\xd0\xb5\xd1\x82 \xd1\x82\xd0\xbe\xd0\xbb\xd1\x8c\xd0\xba\xd0\xbe \xd0\xbd\xd0\xb0 \xd1\x80\xd0\xb5\xd0\xb6\xd0\xb8\xd0\xbc \xc2\xab\xd0\x9f\xd0\xbe \xd0\xbe\xd1\x80\xd1\x83\xd0\xb6\xd0\xb8\xd1\x8e\xc2\xbb. \xd0\x92\xd1\x8b\xd0\xba\xd0\xbb = \xd1\x82\xd1\x8f\xd0\xb6\xd0\xb5\xd1\x81\xd1\x82\xd1\x8c \xd0\xbf\xd0\xbe \xd0\x9a\xd0\x90\xd0\xa2\xd0\x95\xd0\x93\xd0\x9
        0, 12, 1, &g_state.heavy_category);

    regInt(api, apiSize, mod, "heavy_weight",
        tr("Heavy weight threshold", "\xd0\x9f\xd0\xbe\xd1\x80\xd0\xbe\xd0\xb3 \xd1\x82\xd1\x8f\xd0\xb6\xd1\x91\xd0\xbb\xd0\xbe\xd0\xb3\xd0\xbe (\xd0\xb2\xd0\xb5\xd1\x81)"),
        tr("Weapon weight above which it counts as heavy in the 'By weapon' mode.",
           "\xd0\x92\xd0\xb5\xd1\x81 \xd0\xbe\xd1\x80\xd1\x83\xd0\xb6\xd0\xb8\xd1\x8f, \xd0\xb2\xd1\x8b\xd1\x88\xd0\xb5 \xd0\xba\xd0\xbe\xd1\x82\xd0\xbe\xd1\x80\xd0\xbe\xd0\xb3\xd0\xbe \xd0\xbe\xd0\xbd\xd0\xbe \xd1\x81\xd1\x87\xd0\xb8\xd1\x82\xd0\xb0\xd0\xb5\xd1\x82\xd1\x81\xd1\x8f \xd1\x82\xd1\x8f\xd0\xb6\xd1\x91\xd0\xbb\xd1\x8b\xd0\xbc \xd0\xbf\xd1\x80\xd0\xb8 \xc2\xab\xd0\x9f\xd0\xbe \xd0\xbe\xd1\x80\xd1\x83\xd0\xb6\xd0\xb8\xd1\x8e\xc2\xbb."),
        0, 50, 1, &g_state.heavy_weight);

    regBool(api, apiSize, mod, "allow_block_fallback",
        tr("Block after failed dodge", "\xd0\x91\xd0\xbb\xd0\xbe\xd0\xba \xd0\xbf\xd0\xbe\xd1\x81\xd0\xbb\xd0\xb5 \xd1\x83\xd0\xba\xd0\xbb\xd0\xbe\xd0\xbd\xd0\xb0"),
        tr("Dodge failed: also try to block.", "\xd0\xa3\xd0\xba\xd0\xbb\xd0\xbe\xd0\xbd \xd0\xbd\xd0\xb5 \xd1\x83\xd0\xb4\xd0\xb0\xd0\xbb\xd1\x81\xd1\x8f: \xd0\xbf\xd0\xbe\xd0\xbf\xd1\x80\xd0\xbe\xd0\xb1\xd0\xbe\xd0\xb2\xd0\xb0\xd1\x82\xd1\x8c \xd0\xb1\xd0\xbb\xd0\xbe\xd0\xba."),
        tr("When the primary defence is a DODGE and it fails, also roll a block.\nWhen the primary is a BLOCK, no dodge is tried after it.\nThis is the 'dodge, then block' behaviour as a single toggle.",
           "\xd0\x95\xd1\x81\xd0\xbb\xd0\xb8 \xd0\xbe\xd1\x81\xd0\xbd\xd0\xbe\xd0\xb2\xd0\xbd\xd0\xb0\xd1\x8f \xd0\xb7\xd0\xb0\xd1\x89\xd0\xb8\xd1\x82\xd0\xb0 \xe2\x80\x94 \xd0\xa3\xd0\x9a\xd0\x9b\xd0\x9e\xd0\x9d \xd0\xb8 \xd0\xbe\xd0\xbd \xd0\xbd\xd0\xb5 \xd1\x81\xd1\x80\xd0\xb0\xd0\xb1\xd0\xbe\xd1\x82\xd0\xb0\xd0\xbb, \xd0\xb4\xd0\xbe\xd0\xbf\xd0\xbe\xd0\xbb\xd0\
xb6\xd0\xbd\xd0\xb8\xd0\xb5 \xd0\xbf\xd1\x80\xd0\xbe\xd1\x82\xd0\xb8\xd0\xb2\xd0\xbd\xd0\xb8\xd0\xba\xd0\xb8, \xd0\xbe\xd0\xbd \xd0\xb8\xd0\xb3\xd0\xbd\xd0\xbe\xd1\x80\xd0\xb8\xd1\x80\xd1\x83\xd0\xb5\xd1\x82 \xd0\xb4\xd0\xb0\xd0\xbb\xd1\x8c\xd0\xbd\xd1\x8e\xd1\x8e \xd0\xb7\xd0\xb0\xd1\x89\xd0\xb8\xd1\x82\xd1\x83 \xd0\xb8 \xd1\x84\xd0\xbe\xd0\xba\xd1\x83\xd1\x81\xd0\xb8\xd1\x80\xd1\x83\xd0\xb5\xd1\x82\xd1\x81\xd1\x8f \xd0\xbd\xd0\xb0 \xd0\xb4\xd1\x80\xd0\xb0\xd0\xba\xd0\xb5 (\xd1\x81\xd1\x82\xd1\x80\xd0\xb5\xd0\xbb\xd1\x8b \xd0\xbf\xd0\xbe\xd0\xbf\xd0\xb0\xd0\xb4\xd0\xb0\xd1\x8e\xd1\x82)."),
        &g_state.defend_while_in_melee);

    {
        const EMC_SelectOptionV1* topts = (g_lang == LANG_RU) ? kTurretOptions_ru : kTurretOptions_en;
        regSelect(api, apiSize, mod, "turret_mode",
            tr("Turret operator", "\xd0\x9e\xd0\xbf\xd0\xb5\xd1\x80\xd0\xb0\xd1\x82\xd0\xbe\xd1\x80 \xd1\x82\xd1\x83\xd1\x80\xd0\xb5\xd0\xbb\xd0\xb8"),
            tr("What a character operating a turret does vs arrows.", "\xd0\xa7\xd1\x82\xd0\xbe \xd0\xb4\xd0\xb5\xd0\xbb\xd0\xb0\xd0\xb5\xd1\x82 \xd0\xbf\xd0\xb5\xd1\x80\xd1\x81\xd0\xbe\xd0\xbd\xd0\xb0\xd0\xb6 \xd0\xb7\xd0\xb0 \xd1\x82\xd1\x83\xd1\x80\xd0\xb5\xd0\xbb\xd1\x8c\xd1\x8e \xd0\xbf\xd1\x80\xd0\xbe\xd1\x82\xd0\xb8\xd0\xb2 \xd1\x81\xd1\x82\xd1\x80\xd0\xb5\xd0\xbb."),
            tr("A turret operator is bound to the turret.\nNo defence (default): takes the hit.\nDodge / Block: defends with that type (still no defence during its own shot if 'No defence mid-attack' is on).",
               "\xd0\x9e\xd0\xbf\xd0\xb5\xd1\x80\xd0\xb0\xd1\x82\xd0\xbe\xd1\x80 \xd1\x82\xd1\x83\xd1\x80\xd0\xb5\xd0\xbb\xd0\xb8 \xd0\xbf\xd1\x80\xd0\xb8\xd0\xba\xd0\xbe\xd0\xb2\xd0\xb0\xd0\xbd \xd0\xba \xd0\xbd\xd0\xb5\xd0\xb9.\n\xd0\x9d\xd0\xb5 \xd0\xb7\xd0\xb0\xd1\x89\xd0\xb8\xd1\x89\xd0\xb0\xd0\xb5\xd1\x82\xd1\x81\xd1\x8f (\xd0\xbf\xd0\xbe \xd1\x83\xd0\xbc\xd0\xbe\xd0\x
x8f \xd0\xb4\xd0\xb0\xd0\xbb\xd1\x8c\xd0\xbd\xd0\xb5\xd0\xb9 \xd0\xb7\xd0\xb0\xd1\x89\xd0\xb8\xd1\x82\xd1\x8b \xd0\xad\xd0\xa2\xd0\x9e\xd0\x93\xd0\x9e \xd0\xbc\xd0\xbe\xd0\xb4\xd0\xb0.\n\xd0\x9d\xd1\x83\xd0\xb6\xd0\xbd\xd0\xb0 \xd0\xbf\xd1\x80\xd0\xbe\xd0\xb2\xd0\xb5\xd1\x80\xd0\xba\xd0\xb0 \xd0\xb2 \xd0\xb8\xd0\xb3\xd1\x80\xd0\xb5; \xd0\xbf\xd0\xbe \xd1\x83\xd0\xbc\xd0\xbe\xd0\xbb\xd1\x87\xd0\xb0\xd0\xbd\xd0\xb8\xd1\x8e \xd0\xb2\xd1\x8b\xd0\xba\xd0\xbb."),
    ...
    regBool(api, apiSize, mod, "debug_logging",
        ...
        tr("Verbose continuous log next to the DLL: every ranged hit, chosen defence, chances, outcome.\nTemporary aid; turn off when satisfied.",
           "\xd0\x9f\xd0\xbe\xd0\xb4\xd1\x80\xd0\xbe\xd0\xb1\xd0\xbd\xd1\x8b\xd0\xb9 \xd0\xbd\xd0\xb5\xd0\xbf\xd1\x80\xd0\xb5\xd1\x80\xd1\x8b\xd0\xb2\xd0\xbd\xd1\x8b\xd0\xb9 \xd0\xbb\xd0\xbe\xd0\xb3 \xd1\x80\xd1\x8f\xd0\xb4\xd0\xbe\xd0\xbc \xd1\x81 DLL: \xd0\xba\xd0\xb0\xd0\xb6\xd0\xb4\xd0\xbe\xd0\xb5 \xd0\xb4\xd0\xb0\xd0\xbb\xd1\x8c\xd0\xbd\xd0\xb5\xd0\xb5 \xd0\xbf\xd0\xbe\xd0\xbf\xd0\xb0\xd0\xb4\xd0\xb0\xd0\xbd\xd0\xb8\xd0\xb5, \xd0\xb2\xd1\x8b\xd0\xb1\xd1\x80\xd0\xb0\xd0\xbd\xd0\xbd\xd0\xb0\xd1\x8f \xd0\xb7\xd0\xb0\xd1\x89\xd0\xb8\xd1\x82\xd0\xb0, \xd1\x88\xd0\xb0\xd0\xbd\xd1\x81\xd1\x8b, \xd0\xb8\xd1\x81\xd1\x85\xd0\xbe\xd0\xb4.\n\xd0\x92\xd1\x80\xd0\xb5\xd0\xbc\xd0\xb5\xd0\xbd\xd0\xbd\xd0\xbe; \xd0\xb2\xd1\x8b\xd0\xba\xd0\xbb\xd1\x8e\xd1\x87\xd0\xb8, \xd0\xba\xd0\xbe\xd0\xb3\xd0\xb4\xd0\xb0 \xd1\x83\xd0\xb1\xd0\xb5\xd0\xb4\xd0\xb8\xd1\x88\xd1\x8c\xd1\x81\xd1\x8f."),
...
__declspec(dllexport) void startPlugin() {
    ...
    // Log next to the DLL (the mod's own folder — writable on a normal NexusMods/manual install).
    g_log = new std::ofstream((moduleDir() + "\\RangedDefence_log.txt").c_str(), std::ios::out | std::ios::trunc);
    rdlog("RangedDefence v10 GOG loaded.");
